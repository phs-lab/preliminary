...
* 2021-04-11 의견 : 일찍 자고, 일찍 일어나자. 나만의 방식(Routine, Daily Planner)을 만들자. 새벽에는 "[영향력의 원](https://blog.daum.net/streamwk/118)"에 집중하자.
...
<details><summary>야마구치 마유(山口眞由)</summary>
<details><summary>7번 읽기 공부 실천법</summary>

* 책 소개
  + 교보문고 구분 : 자기 계발 > 성공/처세 > 자기관리/처세
  + 야마구치 마유 지음 | 이아랑 옮김 | 한국경제신문사 | 2015년 08월 25일 출간 | 9788947540339
  + 원서 : 東大首席弁護士が實踐!誰でもできる<完全獨學>勉强術 / 山口眞由
* 전체 요약
  + 7번 읽기가 힘들면 "2번째, 4번째, 6번째읽기"로 요약/축약해서 진행해도 사람에 따라서는 효과적이다. 
  + 읽는 방법이 바뀌면(1,2 ~ 3 ~ 4,5 ~ 6,7) 잠시 시간을 두고 다음으로 넘어가는 것이 효과적이다. 
  + 7번을 내리 읽지 않는다. 7번을 몰아서 읽으면 단계가 바뀌어도 방금 읽은 방법과 똑같이 읽게 되기 쉽다. 한 교과서를 한 번 읽은 다음에는 다른 과목 교과서를 읽는다. 하루 이틀 정도 간격을 두면 좋다. 
  + 애초에 입력이 되어 있지 않으면 출력할 수 없다. 우선 입력하는 데 집중하여 교과서를 7번 읽는다.
* 0번 째 읽기 : 【의견】 목차, 머리말을 읽고, 내용은 제목만 읽는다.
* 1번 째 읽기 : 우선 전체를 훑어본다 - 서치라이트를 비추며 대략적인 인상을 수용(1~3읽기 공통)하면서 책장을 넘긴다.
  + 읽지 말고 본문 전체를 훑어본다. ☞ 여러 번 읽는 것에 대한 스트레스를 줄이는 목적인 듯...
    - 【저자 경험】처음과 끝의 목차와 연표는 제외하고 307쪽 국사교과서를 훑어보는데 걸리는 시간은 25분이다.<Br>
      기본적으로 한 쪽에 4초씩, 책의 펼침면을 8초만에 훑어본다.<Br>
      펼침면에 있는 2쪽을 훑어보면서 8초마다 계속해서 책장을 넘겨야 한다. 미리 준비해서 책장을 넘긴다.
    - 1~3번째에 의식할 점은 '훑어보기만 해도 괜찮으니 전체를 파악'하는 것이다.<Br>
      특히 1번째에는 '전혀 이해하지 못해도' 괜찮다. 내용을 이해하려고 애쓰지 않아도 된다.<Br>
      아니 오히려 이해하려고 들지 않는 편이 좋다. 전체를 가볍게 훑어보는 것이 가장 중요하기 때문이다.
</details>
<details><summary>스토리식 기억법</summary>
