...
* 전체 요약 : 7번 읽기가 힘들면 "2번째, 4번째, 6번째읽기"로 요약/축약해서 진행해도 사람에 따라서는 효과적이다. <Br>읽는 방법이 바뀌면(1,2 ~ 3 ~ 4,5 ~ 6,7) 잠시 시간을 두고 다음으로 넘어가는 것이 효과적이다. <Br>7번을 내리 읽지 않는다. 7번을 몰아서 읽으면 단계가 바뀌어도 방금 읽은 방법과 똑같이 읽게 되기 쉽다. 한 교과서를 한 번 읽은 다음에는 다른 과목 교과서를 읽는다. 하루 이틀 정도 간격을 두면 좋다. <Br>애초에 입력이 되어 있지 않으면 출력할 수 없다. 우선 입력하는 데 집중하여 교과서를 7번 읽는다.
