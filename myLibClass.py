...
class my:  # import myLibClass; my = myLibClass.myLib()
  ...
  ## ------------------------------------------------------------------------------------------- ##
  def npArrInfo(ndarrayObj, itemRelDesc=False):  # np.info, np.source, np.lookfor
      """
  ...
  ## ------------------------------------------------------------------------------------------- ##
  # from IPython.display import Markdown, display, Image
  def printbmd(string):  # print black & white text markdown print
      display(Markdown(string))

  ## ------------------------------------------------------------------------------------------- ##
  def printcmd(string, color=None):  # print color text markdown print
  ...
  ## ------------------------------------------------------------------------------------------- ## 일부 오류 있음. 
  def ismutable(aObj):   # https://docs.python.org/ko/3/library/stdtypes.html#immutable-sequence-types
  ...
  ## ------------------------------------------------------------------------------------------- ##
  def viewitems(obj, frPtr=0, toPtr=-1, dict_key=True):
  ...
  def runSQL(sqlstr=None, cursor=None, params=None, 
  ...
  ## ------------------------------------------------------------------------------------------- ##
  def ChooseRightSklearnEstimator(data_cnt=0,
  ...
  ## ------------------------------------------------------------------------------------------- ##
  def sql2pd(inputSQL="SELECT ...", printPandasScript=True, printDebugInfo=False): 
...
if importlib.util.find_spec("rpy2"):
    ...
    print("%Rget, %R -i, %Rpush ⇔ %R -o, %Rpull ☞ 『df』 ① py ⇒ r: ro.r.assign('R.df',ro.pandas2ri.py2ri(PY_df)), ② r ⇒ py: PY_df=ro.pandas2ri.ri2py(R.df)")
